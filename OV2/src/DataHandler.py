...
class DataHandler:
    ...
    def read_activities_and_track_points(self):
        act_temp_id = 1
            ...
            for obj in os.scandir(os.path.join("resources/dataset/Data", user, "Trajectory")):
                if obj.is_file():
                    with open(obj.path) as file:
                        ...
                        self.read_track_points(lines[6:], act_temp_id)
                        act_temp_id += 1
        print(f"Total activities: {len(self.activities)}")
        print(f"Total trackpoints: {sum([len(val) for val in self.track_points.values()])}")
    
    def read_track_points(self, lines, cur_id):
        for line in lines:
            lat, lon, _, altitude, data_days, data_date, data_time = [l.strip() for l in line.split(",")]
            if cur_id not in self.track_points:
                self.track_points[cur_id] = []
            self.track_points[cur_id].append([float(lat), float(lon), int(math.floor(float(altitude))), float(data_days), f"{data_date} {data_time}"])

    def read_data(self):
        print("Starting to read data, this might take a while...")
        self.read_users()
        self.read_activities_and_track_points()
        print("Complete!")
