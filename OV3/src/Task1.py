...
class Task1:
    ...
    def _insert_users(self):
        self.db_controller.insert_many("user", list(self.data.users.apply(lambda row: {"_id": row["id"], "has_labels": row["has_labels"], "activities": self.data.activities[self.data.activities["user_id"] == row["id"]].index.to_list()}, axis=1).to_numpy()))
        
    def _insert_activities(self):
        self.db_controller.insert_many("activity", list(self.data.activities.apply(lambda row: {"_id": row.name, "transportation_mode": row["transportation_mode"], "start_date_time": row["start_date_time"], "end_date_time": row["end_date_time"], "track_points": self.data.track_points[self.data.track_points["activity_id"] == row.name].drop(columns=["activity_id"]).to_dict("records")}, axis=1).to_numpy()))
